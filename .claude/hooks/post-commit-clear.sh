#!/bin/bash
# Post-commit hook: Clear seo-optimization state after successful commit
# This ensures the next commit requires a fresh SEO review
...
if command -v jq &> /dev/null; then
    ...
    if echo "$COMMAND" | grep -qE '^git\s+commit'; then
        "$STATE_MANAGER" clear seo-optimization >/dev/null 2>&1
    fi
else
